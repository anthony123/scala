...
abstract class BCodeTypes extends SubComponent with BytecodeWriters {
  ...
  final val bcodeLock = new _root_.java.util.concurrent.locks.ReentrantLock()
  final def acquire() { bcodeLock.lock()   }
  final def release() { bcodeLock.unlock() }
  ...
  def brefType(iname: String): BType = {
    acquire()
      val tn = newTypeName(iname.toCharArray(), 0, iname.length())
    release()
    brefType(tn)
  }
   ...
   * @must-single-thread
   **/
  def syncIStaticModule(sym: Symbol): Boolean = {
    acquire()
      val b = sym.isModuleClass && !sym.isImplClass && !sym.isLifted
    release()
    b
  }
  ...
  trait BCInnerClassGen {
     ...
     *  @can-multi-thread
     **/
    final def internalName(sym: Symbol): String = { asmClassType(sym, innerClassBufferASM).getInternalName }
     ...
     *  @can-multi-thread
     */
    final def descriptor(t: Type): String = { toTypeKind(t, innerClassBufferASM).getDescriptor }
     ...
     *  @can-multi-thread
     */
    final def descriptor(sym: Symbol): String = { asmClassType(sym, innerClassBufferASM).getDescriptor }
  ...
  trait BCAnnotGen extends BCInnerClassGen {
        ...
        case LiteralAnnotArg(const) =>
          if(const.isNonUnitAnyVal) { av.visit(name, const.value) }
            ...
            const.tag match {
              ...
              case ClazzTag   => av.visit(name, toTypeKind(const.typeValue, innerClassBufferASM).toASMType)
              case EnumTag =>
    ...
    private def shouldEmitAnnotation(annot: AnnotationInfo) = {
      acquire()
      val b = annot.symbol.initialize.isJavaDefined &&
  ...
  trait BCForwardersGen extends BCAnnotGen with BCJGenSigGen {
    ...
    private def addForwarder(isRemoteClass: Boolean, jclass: asm.ClassVisitor, module: Symbol, m: Symbol) {
      ...
      val paramJavaTypes: List[BType] = toTypeKind(methodInfo.paramTypes, innerClassBufferASM)
      ...
      val jReturnType = toTypeKind(methodInfo.resultType, innerClassBufferASM)
      ...
      mirrorMethod.visitMethodInsn(asm.Opcodes.INVOKEVIRTUAL, moduleName, mirrorMethodName, asmMethodType(m, innerClassBufferASM).getDescriptor)
    ...
    def getEnclosingMethodAttribute(clazz: Symbol): EnclMethodEntry = { // JVMS 4.7.7
          ...
          def newEEE(eClass: Symbol, m: Symbol) = {
            EnclMethodEntry(
              ...
              asmMethodType(m, innerClassBufferASM)
            )
          }
  ...
  trait JAndroidBuilder {
    ...
    def legacyAddCreatorCode(clinit: asm.MethodVisitor, cnode: asm.tree.ClassNode, thisName: String) {
      ...
      val androidCreatorType = asmClassType(AndroidCreatorClass, innerClassBufferASM)
   ...
   *  @can-multi-thread
   **/
  final def asmClassType(sym: Symbol, innerClassBufferASM: mutable.Set[BType]): BType = {
   ...
   * @must-single-thread
   **/
  final def toTypeKind(ts: List[Type], innerClassBufferASM: mutable.Set[BType]): List[BType] = {
    ts map { t => toTypeKind(t, innerClassBufferASM) }
  }
   ...
   * @can-multi-thread
   **/
  final def toTypeKind(t: Type, innerClassBufferASM: mutable.Set[BType]): BType = {
    ...
    nrmlzed match {
      ...
      case _: ConstantType    =>
        acquire()
          val u = t.underlying
        release()
        toTypeKind(u, innerClassBufferASM)

      case TypeRef(_, sym, args)    =>
        if(sym == ArrayClass) arrayOf(toTypeKind(args.head, innerClassBufferASM))
      ...
      case AnnotatedType(_, w, _) => toTypeKind(w, innerClassBufferASM) // TODO test/files/jvm/annotations.scala causes an AnnotatedType to reach here.
